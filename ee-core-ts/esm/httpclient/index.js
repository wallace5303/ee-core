...
import eggErrors from "egg-errors";
import Conf from "../config/cache.js";
import Log from "../log/index.js";
import Time from "../utils/time/index.js";
const HttpsAgent = { HttpsAgent: Agent }.HttpsAgent;
const { FrameworkBaseError } = eggErrors;
...
function normalizeConfig(httpConfig) {
    ...
    if (config.httpAgent.timeout < 30000) {
        Log.coreLogger.warn('[ee-core] [httpclient] config.httpclient.httpAgent.timeout(%s) can\'t below 30000, auto reset to 30000', config.httpAgent.timeout);
        config.httpAgent.timeout = 30000;
    }
    if (config.httpsAgent.timeout < 30000) {
        Log.coreLogger.warn('[ee-core] [httpclient] config.httpclient.httpsAgent.timeout(%s) can\'t below 30000, auto reset to 30000', config.httpsAgent.timeout);
